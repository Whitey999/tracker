    ...
    def parsed_date(r):
        ...
        for fmt in ("%Y-%m-%d", "%m/%d/%Y"):
            try:
                ...
                continue
        return None  # unparseable rows sort first (oldest), never crash

    rows = [r for r in rows if r.get("usd_mmk_buy")]  # skip blank/malformed rows
    if not rows:
        return None, False
    rows.sort(key=lambda r: parsed_date(r) or date.min)
    last = rows[-1]
    rate = float(last["usd_mmk_buy"])
    last_date = parsed_date(last)
    return rate, (last_date is not None and last_date.isoformat() == today_str)
